...
function ModalReviewRequestDate({ onclose, open, influencer, campaignId }: ModalReviewRequestDateProps) {
  ...
  const handleApproveDeadline = () => {
    ...
    approveDeadlineFromInfluencer(campaignId, influencer.id as string)
      .then(() => {
  ...
  const handleRequestNewDeadline = () => {
    ...
    requestNewDeadlineForInfluencer(campaignId, influencer.id as string, dayjs(date).toISOString())
      .then(res => {
        ...
        <div className='mt-8'>
          ...
          <div className='flex items-center justify-between h-[72px] rounded-xl p-4 border border-gray-100'>
            <div className='flex gap-3 items-center'>
              ...
              <span className='text-sm font-medium text-gray-800'>{influencer?.name}</span>
            </div>
            <div className='flex flex-col'>
              <span className='text-sm font-normal text-gray-500'>Content Deadline</span>
              <span className='text-sm font-normal text-gray-800'>12/23/2024</span>
            </div>
          </div>
          ...
          <div className='mt-4 h-[56px] flex items-center gap-3 bg-blue-100 rounded-xl p-4'>
            ...
            <span className='font-normal text-sm text-gray-800'>
              ...
              <span className='font-semibold text-sm text-blue-500 ml-1'>12/12/20234</span>
            </span>
          </div>

          <div className='w-full h-[1px] my-[28px] bg-gray-200'></div>
          <span className='text-sm font-bold mb-2 text-gray-800'>Reason</span>
          <p className='text-sm mb-6 font-normal text-gray-500'>Tokens will be locked upon activating the campaign to ensure sufficient funds for campaign operations and influencer payments </p>
        </div>
