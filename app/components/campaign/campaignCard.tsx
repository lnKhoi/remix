...
function CampaignCard({ campaign, onReload }: CampaignCardProps) {
    ...
    return (
        <div>
            <div
                onClick={(e) => {
                    if (hasPermission('view-campaign')) {
                        navigate(`/manager/${campaign.id}`)
                ...
                <div>
                    <h5 className='text-sm text-[#1F2937] truncate font-medium'>
                        {campaign.name}
                    </h5>
                </div>
                ...
                <div className='flex items-center'>
                    {campaign.joinedCreators?.length === 0 && (
                        <button
                            onClick={(e) => { setIsModal(true); e.stopPropagation() }}
                            className='bg-[#F3F4F6] hover:bg-[#D1D5DB] transition-all flex items-center justify-center gap-1 text-sm h-[36px] w-[87px] font-semibold rounded-[8px] text-[#1F2937]'>
